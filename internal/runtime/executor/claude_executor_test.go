...
func TestClaudeExecutor_ensureMaxTokensForThinking(t *testing.T) {
	tests := []struct {
		name      string
		modelName string
		body      string
		...
		{
			name:      "body without thinking returns unchanged",
			modelName: "claude-3-5-sonnet",
			body:      `{"model":"claude-3-5-sonnet","max_tokens":1000}`,
			wantCheck: func(t *testing.T, result []byte) {
				if !strings.Contains(string(result), `"max_tokens":1000`) {
					t.Error("max_tokens should remain 1000")
				}
			},
		},
		{
			name:      "thinking disabled returns unchanged",
			modelName: "claude-sonnet-4-5-thinking",
			body:      `{"model":"claude-sonnet-4-5-thinking","max_tokens":1000,"thinking":{"type":"disabled"}}`,
			wantCheck: func(t *testing.T, result []byte) {
				if !strings.Contains(string(result), `"max_tokens":1000`) {
					t.Error("max_tokens should remain 1000 when thinking is disabled")
				}
			},
		},
		{
			name:      "thinking enabled with zero budget returns unchanged",
			modelName: "claude-sonnet-4-5-thinking",
			body:      `{"model":"claude-sonnet-4-5-thinking","max_tokens":1000,"thinking":{"type":"enabled","budget_tokens":0}}`,
			wantCheck: func(t *testing.T, result []byte) {
				if !strings.Contains(string(result), `"max_tokens":1000`) {
					t.Error("max_tokens should remain 1000 when budget is 0")
				}
			},
		},
		{
			name:      "thinking enabled with budget and low max_tokens is adjusted",
			modelName: "claude-sonnet-4-5-thinking",
			body:      `{"model":"claude-sonnet-4-5-thinking","max_tokens":1000,"thinking":{"type":"enabled","budget_tokens":5000}}`,
			wantCheck: func(t *testing.T, result []byte) {
				// max_tokens should be increased to accommodate budget
				if strings.Contains(string(result), `"max_tokens":1000`) {
					t.Error("max_tokens should be increased when budget_tokens is higher")
				}
			},
		},
		{
			name:      "thinking enabled with adequate max_tokens is unchanged",
			modelName: "claude-sonnet-4-5-thinking",
			body:      `{"model":"claude-sonnet-4-5-thinking","max_tokens":100000,"thinking":{"type":"enabled","budget_tokens":5000}}`,
			wantCheck: func(t *testing.T, result []byte) {
				if !strings.Contains(string(result), `"max_tokens":100000`) {
					t.Error("max_tokens should remain 100000 when it's already sufficient")
				}
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := ensureMaxTokensForThinking(tt.modelName, []byte(tt.body))
			tt.wantCheck(t, result)
		})
	}
}
