...
func TestCheckSystemInstructionsWithMode_StringSystemPreserved(t *testing.T) {
	...
	if !strings.Contains(blocks[2].Get("text").String(), helps.ClaudeCodeIntro) {
		t.Fatalf("blocks[2] should be the static Claude Code prompt block, got %q", blocks[2].Get("text").String())
	}
	if got := gjson.GetBytes(out, "messages.0.content").String(); !strings.Contains(got, "You are a helpful assistant.") {
		t.Fatalf("expected first user message to carry forwarded system prompt, got %q", got)
	}
}
...
func TestCheckSystemInstructionsWithMode_ArraySystemStillWorks(t *testing.T) {
	...
	if got := gjson.GetBytes(out, "messages.0.content").String(); !strings.Contains(got, "Be concise.") {
		t.Fatalf("expected first user message to carry forwarded array system prompt, got %q", got)
	}
}
...
func TestApplyCloaking_PreservesConfiguredStrictModeAndSensitiveWordsWhenModeOmitted(t *testing.T) {
	...
	if got := gjson.GetBytes(out, "messages.0.content.0.text").String(); strings.Contains(got, "<system-reminder>") {
		t.Fatalf("strict mode should not prepend forwarded system prompt, got %q", got)
	}
}
