...
// EstimateRequestTokens estimates total tokens for a request payload.
// It auto-detects the format (OpenAI chat, Claude, etc.) and counts accordingly.
func EstimateRequestTokens(model string, payload []byte) (int64, error) {
	...
	// Detect format and count accordingly
	...
	// OpenAI responses format or other - use general text counting
...
// CalculateCompressionBudget determines how many tokens should be compressed
// to bring context usage under the target percentage.
func CalculateCompressionBudget(currentTokens int64, contextWindow int, targetPercent float64) int64 {
	if contextWindow <= 0 || targetPercent <= 0 || targetPercent >= 1 {
		return 0
	}
	targetTokens := int64(float64(contextWindow) * targetPercent)
...
// ShouldTriggerCompression checks if compression should be triggered based on
// current token usage vs context window size.
func ShouldTriggerCompression(currentTokens int64, contextWindow int, thresholdPercent float64) bool {
	if contextWindow <= 0 || thresholdPercent <= 0 {
		return false
	}
	ratio := float64(currentTokens) / float64(contextWindow)
	return ratio >= thresholdPercent
}

// GetDefaultContextWindow returns a conservative default context window for a model.
