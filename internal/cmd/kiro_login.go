...
// DoAmazonQImport imports token from Amazon Q CLI's SQLite database.
// Amazon Q CLI has separate usage quota from Kiro IDE.
//
// Parameters:
//   - cfg: The application configuration
//   - options: Login options (currently unused for import)
func DoAmazonQImport(cfg *config.Config, options *LoginOptions) {
	if options == nil {
		options = &LoginOptions{}
	}
	...
	// Use AmazonQAuthenticator to import from CLI
	authenticator := sdkAuth.NewAmazonQAuthenticator()
	record, err := authenticator.ImportFromAmazonQCLI(context.Background(), cfg)
	if err != nil {
		log.Errorf("Amazon Q CLI token import failed: %v", err)
		fmt.Println("\nMake sure Amazon Q CLI is installed and authenticated:")
		fmt.Println("1. Install Amazon Q CLI (see: https://docs.aws.amazon.com/amazonq/)")
		fmt.Println("2. Run: q login")
		fmt.Println("3. Complete authentication in browser")
		fmt.Println("4. Run this command again")
		return
	}
	...
	fmt.Println("Amazon Q CLI token imported successfully!")
